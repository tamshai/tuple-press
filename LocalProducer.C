...
// Not absolutely necessary, but for consistency
    return (Int_t)cmult;
}
...
//by  b_branchname->GetEntry(ientry); //read only this branch
    ...
    for (Long64_t jentry = 0; jentry != nentries; ++jentry) {
        ...
        for (Int_t i = 0; i != PFJetsCHS__; ++i) {
            ...
            if (p4.Pt() > minPt) {
                ...
                if (i_out < ncomp) {
                    ...
                    //std::cout << PFJetsCHS__hadronFlavour_[i] << ' ' << (int)(hfl[i_out]) << ' ' <<  hfl[i_out] << std::endl;
		...
		// Ugly hack to save the number of jets
